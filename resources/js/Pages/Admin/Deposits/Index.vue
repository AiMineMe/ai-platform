...
const props = defineProps({
    ...
    stats: {
        ...
        validator: (value) => {
            return value && typeof value === 'object' &&
                typeof value.totalDeposits === 'number' &&
                typeof value.pendingDeposits === 'number' &&
...
const updatingDeposit = ref(null);
...
const totalDepositsCount = computed(() => props.stats?.total_deposits || 0);
const pendingDepositsCount = computed(() => props.stats?.pending_deposits || 0);
...
const applyFilters = () => {
    ...
    router.get('/admin/deposits', params, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
        onFinish: () => {
...
const getGatewayName = (deposit) => {
    const gateway = deposit.payment_gateway || deposit.paymentGateway;
    return (gateway && gateway.name) ? gateway.name : 'N/A';
};

const getGatewayType = (deposit) => {
    const gateway = deposit.payment_gateway || deposit.paymentGateway;
    return (gateway && gateway.type) ? gateway.type : null;
};
const getUserName = (deposit) => {
    return (deposit.user && deposit.user.name) ? deposit.user.name : 'N/A';
};
...
const getTypeClass = (type) => {
    const classes = {
        ...
        'automatic': 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300'
    };
...
const handleSort = (field) => {
    ...
    router.get('/admin/deposits', params, {
        ...
        onError: (errors) => {
            ...
            error.value = 'Failed to sort deposits. Please try again.';
            showToast('Failed to sort deposits', 'error');
        }
    });
};
...
const canUpdateStatus = (deposit) => {
    if (!isAdmin.value) return false;
    if (!deposit || deposit.status !== 'pending') return false;
    return getGatewayType(deposit) === 'manual';
...
const confirmApprove = () => {
    if (!selectedDeposit.value || isProcessing.value) return;

    isProcessing.value = true;

    router.patch(`/admin/deposits/${selectedDeposit.value.id}/approve`, {
...
const confirmReject = () => {
    if (!selectedDeposit.value || isProcessing.value || !rejectResponse.value.trim()) return;

    isProcessing.value = true;

    router.patch(`/admin/deposits/${selectedDeposit.value.id}/reject`, {
...
const updateDepositStatus = async (depositId, newStatus) => {
    if (!isAdmin.value || updatingDeposit.value === depositId) return;

    const deposit = props.deposits.find(d => d && d.id === depositId);
    if (!deposit || !canUpdateStatus(deposit)) {
        showToast('You do not have permission to update this deposit\'s status', 'error');
        return;
    }

    updatingDeposit.value = depositId;
    error.value = null;

    try {
        router.patch(`/admin/deposits/${depositId}/${newStatus}`, {}, {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => {
                showToast(`Deposit ${newStatus} successfully`, 'success');
            },
            onError: (errors) => {
                console.error('Status update error:', errors);
                const errorMessage = errors.message || `Failed to ${newStatus} deposit`;
                error.value = errorMessage;
                showToast(errorMessage, 'error');
            },
            onFinish: () => {
                updatingDeposit.value = null;
            }
        });
    } catch (error) {
        console.error(`Error ${newStatus} deposit:`, error);
        showToast(`Failed to ${newStatus} deposit`, 'error');
        updatingDeposit.value = null;
    }
};
    ...
    >
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                    ...
                    <tr v-if="!isLoading && deposits.length === 0">
                        <td
                            colspan="7"
                        ...
                        >
                            <div class="text-center">
                                ...
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                    No deposit requests match your current filters. Try adjusting the search criteria.
                                </p>
                            </div>
                        </td>
                    </tr>
                    ...
                    <tr v-if="isLoading">
                        ...
                        >
                            <div class="flex items-center justify-center">
                                ...
                                <span class="text-gray-600 dark:text-gray-400">Loading deposits...</span>
                            </div>
                        </td>
                    </tr>
                            ...
                            <div class="flex items-center space-x-3">
                                <div class="h-8 w-8 flex-shrink-0">
                                    <div
                                        ...
                                        {{ getInitials(deposit.user && deposit.user.name ? deposit.user.name : null) }}
                                    </div>
                                </div>
                                <div class="min-w-0 flex-1">
                                    <div class="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">
                                        {{ (deposit.user && deposit.user.name) ? deposit.user.name : 'N/A' }}
                                    </div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400 truncate">
                                        {{ (deposit.user && deposit.user.email) ? deposit.user.email : 'N/A' }}
                                    </div>
                                </div>
                            </div>
                        </td>
                  ...
                  >
                    {{ getGatewayType(deposit) || 'N/A' }}
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900 dark:text-gray-100">
                                <div class="font-semibold text-blue-600 dark:text-blue-400">
                                    {{ formatNumber(deposit.final_amount || 0) }} {{ deposit.currency || 'N/A' }}
                                </div>
                                <div class="text-xs text-gray-500 dark:text-gray-400">
                                    Request: {{ formatNumber(deposit.amount || 0) }} {{ deposit.currency || 'N/A' }}
                                </div>
                                <div
                                    v-if="deposit.charge && parseFloat(deposit.charge) > 0"
                                    class="text-xs text-red-500 dark:text-red-400"
                                >
                                    Charge: {{ formatNumber(deposit.charge) }} {{ deposit.currency || 'N/A' }}
                                </div>
                                <div
                                    class="text-xs text-green-600 dark:text-green-400 border-t border-gray-200 dark:border-gray-600 pt-1 mt-1">
                                    Credit: {{ currencySymbol }}{{ formatNumber(deposit.deposit_amount || 0) }}
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div
                                v-if="isAdmin && canUpdateStatus(deposit)"
                                class="min-w-[140px]"
                            >
                                <select
                                    ...
                                    @change="updateDepositStatus(deposit.id, $event.target.value)"
                                >
                                    <option
                  ...
                  <span
                      ...
                      :class="getStatusDotClass(deposit.status)"
                  />
                  {{ formatStatus(deposit.status) }}
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <div
                                v-if="deposit.created_at"
                                class="text-xs text-gray-500 dark:text-gray-400"
                            >
                                {{ getRelativeTime(deposit.created_at) }}
                            </div>
                        </td>

                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center space-x-2">
                                <template v-if="deposit.status === 'pending' && getGatewayType(deposit) === 'manual'">
                                    <button
                                        :disabled="updatingDeposit === deposit.id"
                                        ...
                                        @click="openApproveModal(deposit)"
                                    >
                                        <svg
                                            class="h-3 w-3 mr-1"
                                            fill="none"
        ...
        >
            ...
            >
                <div class="p-6">
                    <div
                        class="flex items-center justify-center w-12 h-12 mx-auto bg-green-100 dark:bg-green-900/30 rounded-full mb-4">
                        <svg
                    ...
                    >
                        ...
                        <div class="space-y-3">
                            <div class="bg-blue-50 dark:bg-blue-900/20 p-3 rounded border-l-4 border-blue-500">
                                <div class="text-xs text-blue-700 dark:text-blue-300 font-medium mb-2">
                                    Payment Gateway ({{ selectedDeposit.currency }})
                                </div>
                                <div class="text-blue-600 dark:text-blue-400 font-semibold">
                                    Total Payment: {{ formatNumber(selectedDeposit.final_amount) }}
                                    {{ selectedDeposit.currency }}
                                </div>
                                <div class="text-gray-600 dark:text-gray-400 text-sm">
                                    Request: {{ formatNumber(selectedDeposit.amount) }} {{ selectedDeposit.currency }}
                                </div>
                                <div
                                    v-if="selectedDeposit.charge && parseFloat(selectedDeposit.charge) > 0"
                                    class="text-red-500 dark:text-red-400 text-sm"
                                >
                                    Charge: {{ formatNumber(selectedDeposit.charge) }} {{ selectedDeposit.currency }}
                                </div>
                            </div>
                            ...
                            <div class="bg-green-50 dark:bg-green-900/20 p-3 rounded border-l-4 border-green-500">
                                ...
                                <div class="text-green-600 dark:text-green-400 font-semibold">
                                    Will Credit: {{ currencySymbol }}{{ formatNumber(selectedDeposit.deposit_amount) }}
                                </div>
                            </div>
                        </div>
                        ...
                        >
                            ...
                            <div class="text-sm space-y-2">
                                ...
                                >
                                    <span class="text-gray-600 dark:text-gray-400 capitalize min-w-[140px]">{{
                                            formatFieldName(key)
                                        }}:</span>
                                    <span
                                        class="font-medium text-gray-900 dark:text-gray-100 break-all text-right flex-1 ml-2">
                    <template v-if="value && key === 'file'">
                      <a
                          :href="`/admin/deposits/${selectedDeposit.id}/view/${key}`"
                          target="_blank"
                    ...
                    <div
                        ...
                        >
                            ...
                            {{ isProcessing ? 'Approving...' : 'Approve Deposit' }}
                        </button>
                        <button
                ...
                <div class="p-6">
                    ...
                    >
                        ...
                        <div class="space-y-3">
                            <div class="bg-red-50 dark:bg-red-900/20 p-3 rounded border-l-4 border-red-500">
                                <div class="text-xs text-red-700 dark:text-red-300 font-medium mb-2">
                                    Payment Gateway ({{ selectedDeposit.currency }})
                                </div>
                            ...
                            <div class="bg-gray-50 dark:bg-gray-700 rounded p-3">
                                <div class="text-xs text-gray-700 dark:text-gray-300 font-medium mb-2">
                                    Amount Not to Credit
                                </div>
                    ...
                    <div
                        ...
                        >
                            ...
                            {{ isProcessing ? 'Rejecting...' : 'Reject Deposit' }}
                        </button>
                        <button
                            class="w-full sm:w-auto inline-flex justify-center items-center px-6 py-2.5 bg-white hover:bg-gray-50 dark:bg-slate-600 dark:hover:bg-slate-500 text-gray-700 dark:text-gray-300 font-medium rounded-lg border border-gray-300 dark:border-slate-500 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                            @click="closeRejectModal"
