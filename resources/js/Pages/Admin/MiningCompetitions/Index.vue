...
const processingCompetition = ref(null);
...
const sortField = computed(() => props.filters.sort_field || 'starts_at');
...
const applyFilters = () => {
    ...
    router.get('/admin/mining-competitions', params, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
        onFinish: () => {
...
const handleSort = (field) => {
    ...
    router.get('/admin/mining-competitions', params, {
        ...
        onError: (errors) => {
            ...
            error.value = 'Failed to sort competitions. Please try again.';
            showToast('Failed to sort competitions', 'error');
        }
    });
};
...
const createCompetition = () => {
    router.get('/admin/mining-competitions/create');
};

const editCompetition = (competition) => {
    router.get(`/admin/mining-competitions/${competition.id}/edit`);
};

const viewCompetition = (competition) => {
    router.get(`/admin/mining-competitions/${competition.id}`);
};
...
const confirmDelete = () => {
    if (!selectedCompetition.value || processingCompetition.value) return;

    processingCompetition.value = selectedCompetition.value.id;

    router.delete(`/admin/mining-competitions/${selectedCompetition.value.id}`, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
            closeDeleteModal();
        ...
        onFinish: () => {
            processingCompetition.value = null;
        }
    });
};
...
const getDaysRemaining = (competition) => {
    const now = new Date();
    const startDate = new Date(competition.starts_at);
    const endDate = new Date(competition.ends_at);
...
<template>
    <AdminLayout
        title="Mining Competitions Management"
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">
                        Mining Competitions Management
                    </h1>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                        Organize and manage mining competitions with prize pools and leaderboards
                    </p>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Total Competitions</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ stats.totalCompetitions }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Active Competitions</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ stats.activeCompetitions }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Total Prize Pool</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ currencySymbol }}{{ formatNumber(stats.totalPrizePool) }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                    ...
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-700 dark:hover:text-gray-300" @click="handleSort('prize_pool')">
                            <div class="flex items-center space-x-1">
                                <span>PRIZE POOL</span>
                                <svg v-if="sortField === 'prize_pool'" class="w-4 h-4" :class="sortDirection === 'asc' ? 'transform rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                                </svg>
                            </div>
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-700 dark:hover:text-gray-300" @click="handleSort('entry_fee')">
                            <div class="flex items-center space-x-1">
                                <span>ENTRY FEE</span>
                                <svg v-if="sortField === 'entry_fee'" class="w-4 h-4" :class="sortDirection === 'asc' ? 'transform rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                                </svg>
                            </div>
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-700 dark:hover:text-gray-300" @click="handleSort('starts_at')">
                            <div class="flex items-center space-x-1">
                                <span>SCHEDULE</span>
                                <svg v-if="sortField === 'starts_at'" class="w-4 h-4" :class="sortDirection === 'asc' ? 'transform rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                                </svg>
                            </div>
                        </th>
                    ...
                    <tr v-if="!isLoading && competitions.length === 0">
                        <td colspan="7" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                            <div class="text-center">
                                ...
                                <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">No competitions found</h3>
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">No competitions match your current filters.</p>
                                <div class="mt-6">
                                    <button
                                        class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-slate-700 hover:bg-gray-50 dark:hover:bg-slate-600 transition-colors duration-200"
                                        @click="createCompetition"
                    ...
                    <tr v-if="isLoading">
                        <td colspan="7" class="px-6 py-12 text-center">
                            <div class="flex items-center justify-center">
                                ...
                                <span class="text-gray-600 dark:text-gray-400">Loading competitions...</span>
                            </div>
                        </td>
                    </tr>

                    <tr v-for="competition in competitions" :key="competition.id" class="hover:bg-gray-50 dark:hover:bg-slate-700">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full border" :class="getTypeClass(competition.type)">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                {{ competition.prizes?.length || 0 }} prize tiers
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                {{ competition.admin_fee_percentage }}% admin fee
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                to {{ formatDate(competition.ends_at) }}
                            </div>
                            <div class="text-xs text-blue-600 dark:text-blue-400 font-medium">
                                {{ getDaysRemaining(competition) }}
                            </div>
                        </td>

                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full border" :class="getStatusClass(competition)">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                {{ competition.max_participants ? `/ ${competition.max_participants} max` : 'unlimited' }}
                            </div>
                        </td>
            ...
            >
                <div class="p-6">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 text-center text-sm mb-4">
                        Are you sure you want to delete this competition? This action cannot be undone and will remove all participant data and leaderboards.
                    </p>
                    <div class="flex flex-col sm:flex-row-reverse sm:space-x-reverse sm:space-x-3 space-y-3 sm:space-y-0">
                        <button
                            :disabled="processingCompetition"
                            ...
                            @click="confirmDelete"
                        >
                            <svg v-if="processingCompetition" class="animate-spin -ml-1 mr-2 h-4 w-4 text-white" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 818-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 714 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/>
                            </svg>
                            {{ processingCompetition ? 'Deleting...' : 'Delete Competition' }}
                        </button>
                        <button
