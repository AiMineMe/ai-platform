...
const props = defineProps({
    ...
    stats: {
        ...
        validator: (value) => {
            return value && typeof value === 'object' &&
                typeof value.totalSales === 'number' &&
                typeof value.completedSales === 'number' &&
...
const totalSalesCount = computed(() => props.stats?.totalSales || 0);
const completedSalesCount = computed(() => props.stats?.completedSales || 0);
...
const applyFilters = () => {
    ...
    router.get('/admin/ico-sales', params, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
        onFinish: () => {
...
const handleSort = (field) => {
    ...
    router.get('/admin/ico-sales', params, {
        ...
        onError: (errors) => {
            ...
            error.value = 'Failed to sort sales. Please try again.';
            showToast('Failed to sort sales', 'error');
        }
    });
};
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            Monitor all user token sales and transaction activities
          </p>
        </div>
      </div>
    </div>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            ...
            <p class="text-2xl font-bold text-gray-900 dark:text-white">
              {{ formatNumber(completedSalesCount) }}
            </p>
          </div>
        </div>
      </div>
          ...
          <div class="flex-1 max-w-md">
            <div class="relative">
              ...
              <input
                ...
                v-model="searchTerm"
                type="text"
                placeholder="Search by user, token, or sale ID..."
          ...
          <thead class="bg-gray-50 dark:bg-slate-900">
            <tr>
              <th
                ...
                @click="handleSort('sale_id')"
              >
                <div class="flex items-center space-x-1">
                  <span>SALE ID</span>
                  <svg
                    v-if="sortField === 'sale_id'"
                    class="w-4 h-4"
              ...
              <th
                class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-700 dark:hover:text-gray-300"
                @click="handleSort('tokens_sold')"
              >
                <div class="flex items-center space-x-1">
                  <span>TOKENS SOLD</span>
                  <svg
                    v-if="sortField === 'tokens_sold'"
                    class="w-4 h-4"
                    :class="sortDirection === 'asc' ? 'transform rotate-180' : ''"
                    fill="none"
                    stroke="currentColor"
              ...
              <th
                class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-700 dark:hover:text-gray-300"
                @click="handleSort('sold_at')"
              >
                <div class="flex items-center space-x-1">
                  <span>DATE</span>
                  <svg
                    v-if="sortField === 'sold_at'"
                    class="w-4 h-4"
                    :class="sortDirection === 'asc' ? 'transform rotate-180' : ''"
                    fill="none"
                    stroke="currentColor"
              ...
              >
                <div class="text-center">
                  ...
                  <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    No token sales match your current filters. Try adjusting the search criteria.
                  </p>
                </div>
              </td>
            </tr>
            ...
            <tr v-if="isLoading">
              ...
              >
                <div class="flex items-center justify-center">
                  ...
                  <span class="text-gray-600 dark:text-gray-400">Loading sales...</span>
                </div>
              </td>
            </tr>
            ...
            >
                ...
                <div class="flex items-center space-x-3">
                  <div class="h-8 w-8 flex-shrink-0">
                    <div class="h-full w-full rounded-full bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-white font-semibold text-xs shadow-md">
                      {{ getTokenInitials(sale.ico_token?.symbol) }}
                    </div>
                  </div>
                  <div class="min-w-0 flex-1">
                    <div class="text-sm font-medium text-gray-900 dark:text-gray-100">
                      {{ sale.ico_token?.name || 'Unknown Token' }}
                    </div>
                    <div class="text-xs text-gray-500 dark:text-gray-400 font-mono">
                      {{ sale.ico_token?.symbol || 'N/A' }}
                    </div>
                  </div>
                </div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm font-semibold text-gray-900 dark:text-gray-100">
                  {{ formatNumber(parseFloat(sale.tokens_sold || 0)) }}
                </div>
                <div class="text-xs text-gray-500 dark:text-gray-400">
                  @ {{ currencySymbol }}{{ formatNumber(parseFloat(sale.sale_price || 0)) }} each
                </div>
              </td>

              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm font-semibold text-gray-900 dark:text-gray-100">
                  {{ currencySymbol }}{{ formatNumber(parseFloat(sale.total_amount || 0)) }}
                </div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                ...
                <div class="text-xs text-gray-500 dark:text-gray-400">
                  {{ formatTime(sale.sold_at || sale.created_at) }}
                </div>
              </td>
            </tr>
          </tbody>
