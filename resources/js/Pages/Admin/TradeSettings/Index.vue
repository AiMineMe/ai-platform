...
const selectedPayoutRange = ref(props.filters?.payout_range || '');
...
const hasActiveFilters = computed(() => {
    return !!(searchTerm.value || selectedStatus.value || selectedPayoutRange.value);
});

const totalSymbolsCount = computed(() => props.stats?.totalSymbols || 0);
const activeSymbolsCount = computed(() => props.stats?.activeSymbols || 0);
const totalActiveTradesCount = computed(() => props.stats?.totalActiveTrades || 0);
const totalVolumeCount = computed(() => props.stats?.totalVolume || 0);
...
const applyFilters = () => {
    ...
    if (selectedPayoutRange.value) params.payout_range = selectedPayoutRange.value;

    router.get('/admin/trade-settings', params, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
        onFinish: () => {
...
const clearPayoutRangeFilter = () => {
    selectedPayoutRange.value = '';
    applyFilters();
};
...
const getPayoutRangeLabel = (range) => {
    const option = payoutRangeOptions.find(p => p.value === range);
...
const getSymbolInitials = (symbol) => {
    if (!symbol || typeof symbol !== 'string') return '??';
    return symbol.substring(0, 2).toUpperCase();
};
...
const getPayoutRateClass = (payoutRate) => {
    const rate = parseFloat(payoutRate) || 0;
    if (rate >= 85) return 'text-green-600 dark:text-green-400';
    if (rate >= 70) return 'text-yellow-600 dark:text-yellow-400';
    return 'text-red-600 dark:text-red-400';
};

const getActiveTradesClass = (activeTrades) => {
    const trades = parseFloat(activeTrades) || 0;
    if (trades > 10) return 'text-green-600 dark:text-green-400';
    if (trades > 0) return 'text-blue-600 dark:text-blue-400';
...
const handleSort = (field) => {
    ...
    router.get('/admin/trade-settings', params, {
        ...
        onError: (errors) => {
            ...
            error.value = 'Failed to sort settings. Please try again.';
            showToast('Failed to sort settings', 'error');
        }
    });
};
...
const createSetting = () => {
    router.get('/admin/trade-settings/create');
};

const editSetting = (setting) => {
    if (!setting || !setting.id) {
        showToast('Invalid setting selected', 'error');
        return;
    }
    router.get(`/admin/trade-settings/${setting.id}/edit`);
};
...
const confirmDelete = () => {
    if (!selectedSetting.value || isProcessing.value) return;

    isProcessing.value = true;

    router.delete(`/admin/trade-settings/${selectedSetting.value.id}`, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
            closeDeleteModal();
            showToast('Trading symbol deleted successfully', 'success');
        },
        onError: () => {
            showToast('Failed to delete trading symbol', 'error');
        },
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
              Manage trade trade parameters like assets, return percentages, and trade timeframes.
          </p>
        </div>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            ...
            <p class="text-2xl font-bold text-gray-900 dark:text-white">
              {{ currencySymbol }}{{ formatMoney(totalVolumeCount) }}
            </p>
          </div>
        </div>
      </div>
          ...
          <div class="flex flex-wrap items-center gap-3">
            ...
            <div class="min-w-[160px]">
              ...
              >
                ...
                <option
                  v-for="payout in payoutRangeOptions"
                  :key="payout.value"
                  :value="payout.value"
                >
                  {{ payout.label }}
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
          <thead class="bg-gray-50 dark:bg-slate-900">
            <tr>
              ...
              <th
                class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-700 dark:hover:text-gray-300"
                @click="handleSort('payout_rate')"
              >
                <div class="flex items-center space-x-1">
                  <span>PAYOUT RATE</span>
                  <svg
                    v-if="sortField === 'payout_rate'"
                    class="w-4 h-4"
                    :class="sortDirection === 'asc' ? 'transform rotate-180' : ''"
                    fill="none"
                    stroke="currentColor"
          ...
          <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
            <tr v-if="!isLoading && tradeSettings.length === 0">
              <td
                colspan="8"
              ...
              >
                <div class="text-center">
                  ...
                  <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">
                    No trading symbols found
                  </h3>
                  <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Get started by creating your first trading symbol configuration.
                  </p>
                </div>
              </td>
            </tr>
            ...
            <tr v-if="isLoading">
              ...
              >
                <div class="flex items-center justify-center">
                  ...
                  <span class="text-gray-600 dark:text-gray-400">Loading trading settings...</span>
                </div>
              </td>
            </tr>

            <tr
              v-for="setting in tradeSettings"
              :key="setting.id"
            ...
            >
                <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center space-x-3">
                        <div class="h-10 w-10 flex-shrink-0 relative">
                            <img
                                v-if="setting.currency.image_url"
                                :src="setting.currency.image_url"
                                :alt="setting.currency.name"
                            ...
                            <div
                                ...
                                :style="{ display: setting.currency.image_url ? 'none' : 'flex' }"
                            >
                                {{ getCurrencyInitials(setting.currency.symbol) }}
                            </div>
                        </div>
                        <div class="min-w-0 flex-1">
                            <div class="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate">
                                {{ setting.currency.name || 'Unknown' }}
                            </div>
                            <div class="text-xs text-gray-500 dark:text-gray-400 font-mono truncate">
                                {{ setting.currency.symbol || 'N/A' }}
                            </div>
                        </div>
                    </div>
                </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span
                  ...
                  :class="getStatusClass(setting.is_active)"
                >
                  <span
                    class="w-2 h-2 rounded-full mr-1"
                    :class="getStatusDotClass(setting.is_active)"
                  />
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-600 dark:text-gray-300">
                  <div class="font-semibold text-gray-900 dark:text-gray-100">
                    {{ currencySymbol }}{{ formatNumber(setting.min_amount) }} -  {{ currencySymbol }}{{ formatNumber(setting.max_amount) }}
                  </div>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-600 dark:text-gray-300">
                  <div
                    class="font-semibold"
                    :class="getPayoutRateClass(setting.payout_rate)"
                  >
                    {{ formatPercentage(setting.payout_rate) }}%
                  </div>
                  <div class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                    Return Rate
                  </div>
              ...
              <td class="px-6 py-4">
                <div class="text-sm text-gray-500 dark:text-gray-400">
                  <div
                    v-if="setting.formatted_durations && setting.formatted_durations.length > 0"
                    class="flex flex-wrap gap-1"
                  >
                    <span
                      v-for="duration in setting.formatted_durations.slice(0, 2)"
                      :key="duration"
                      class="px-2 py-1 bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300 text-xs rounded border border-blue-200 dark:border-blue-700"
                    >
                      {{ duration }}
                    </span>
                    <span
                      v-if="setting.formatted_durations.length > 2"
                      class="px-2 py-1 bg-gray-100 text-gray-600 dark:bg-gray-900/30 dark:text-gray-400 text-xs rounded border border-gray-200 dark:border-gray-600"
                    >
                      +{{ setting.formatted_durations.length - 2 }} more
                    </span>
                  </div>
                  <div
                    v-else
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-600 dark:text-gray-300">
                  <div class="font-medium text-gray-900 dark:text-gray-100">
                    {{ formatNumber(setting.trade_count || 0) }} total
                  </div>
                  <div
                    class="text-xs mt-0.5"
                    :class="getActiveTradesClass(setting.active_trades)"
                  >
                    {{ formatNumber(setting.active_trades || 0) }} active
                  </div>
                </div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-600 dark:text-gray-300">
                  <div class="font-medium text-gray-900 dark:text-gray-100">
                    {{ currencySymbol }}{{ formatMoney(setting.total_volume || 0) }}
                  </div>
                  <div class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                    Total Volume
                  </div>
          ...
          >
              <div class="p-6">
                  ...
                  <p class="text-gray-600 dark:text-gray-400 text-center text-sm mb-4">
                      Are you sure you want to delete this trading symbol? This action cannot be undone and will affect all associated trades.
                  </p>
                  ...
                  <div class="flex flex-col sm:flex-row-reverse sm:space-x-reverse sm:space-x-3 space-y-3 sm:space-y-0">
                      ...
                      >
                          ...
                          {{ isProcessing ? 'Deleting...' : 'Delete Symbol' }}
                      </button>
                      <button
